...
class ResPartner(models.Model):
    ...
    def _compute_factor_data(self):
        for partner in self:
            journals = self.env["account.journal"].with_context(
                    {"compute_factor_partner": partner}
            ).search([("is_factor", "=", True)])
            partner.factor_credit = sum(
                journals.mapped('factor_customer_credit')
            )
            holdback = sum(
                journals.mapped('factor_holdback_balance')
            )
            limit_holdback = sum(
                journals.mapped('factor_limit_holdback_balance')
            )
    ...
    def open_customer_holdback(self):
        ...
        action["domain"] = "[('full_reconcile_id', '=', False), ('account_id', 'in', %s), ('partner_id', '=', %s)]" % (
            accounts, self.id
        )
        return action